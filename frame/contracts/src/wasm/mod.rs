...
pub struct PrefabWasmModule<T: Config> {
	...
	code: Vec<u8>,
	/// This is the amount of storage that is occupied by this module in storage.
	///
	/// It includes the sizes of the instrumented code and the pristine code.
	/// This is the value that should be used in rent calculations.
	occupied_storage: u32,
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	...
	fn occupied_storage(&self) -> u32 {
		...
		let len = self.occupied_storage.saturating_add(size_of::<PrefabWasmModule<T>>() as u32);
